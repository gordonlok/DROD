    ...
    def fit(self):
        ...
        for i in range(self.row):
            if part_map[i] == -1:
                ...
                # 排名策略score = e^(subset_degree) + local_anomaly_score，
                # 排名策略可能与score[i] = subset_degree[part_map[i]]相差不大
                # score[i] = np.exp(subset_degree_rank[part_map[i]]) + local_anomaly_score[i]
...
if __name__ == '__main__':
    ...
    for str1 in filestr:
        ...
        with open(save_path, 'a') as f:
            f.write("Ablation MGOD-0: Dataset: " + str(str1) + " AUC: " + str(auc) + " PR: " + str(pn) + "\n")
